...
for i in range(len(filesA)):
  ...
  #  Point = np.where(np.gradient(vars()[f'FinalY {filesA[i]}']) == min(np.gradient(vars()[f'FinalY {filesA[i]}'])))[0][0]
    ...
    m,intercept,err = F.LineValFinder(hv,vars()[f'FinalY {filesA[i]}'],LamRangeA,'bg',hvMinA,hvMaxA,True)
    yFit = m * hv + intercept
...
for i in range(len(files1A)):
    ...
    plt.figure(23456)
    m1,intercept1,err1 = F.LineValFinder(hv,vars()[f'FinalY {files1A[i]}'],LamRange1A,'bg',hvMin1A,hvMax1A,True)
...
for i in range(len(filesA)):
    ...
    ax2 = fig1.add_axes([0.2,0.45,0.4,0.4])
    ...
    ax1.text(1.0 / (sizeFigSqaure/8), 1.45 / (sizeFigSqaure/8), r'$\alpha-Ga_{2}O_{3}$', horizontalalignment='center',verticalalignment='center', transform=ax.transAxes, color = LineColour[0], fontsize = legendFontSize)
    ax1.text(1.05/ (sizeFigSqaure/8), 0.3 / (sizeFigSqaure/8), r'$\beta-Ga_{2}O_{3}$', horizontalalignment='center',verticalalignment='center', transform=ax.transAxes, color = LineColour[1], fontsize = legendFontSize)
    ax1.tick_params(axis='both', which='major', pad=axesFontPadding)
  

    ax2.plot(hv,vars()[f'FinalY {filesA[i]}'], color = LineColour[0])
    ax2.plot(vars()[filesA[i]+'hvConst'],vars()[filesA[i]+'yFitConst'], label = r'$E_{bg}$ = '+str(np.around(vars()[filesA[i]+'Intercept'][0],2))+r'eV', linestyle = '--', color = LineColour[3])
    ax2.plot(hv,vars()[f'FinalY {files1A[i]}'], color = LineColour[1])
    ax2.plot(vars()[files1A[i]+'hvConst'],vars()[files1A[i]+'yFitConst'], label = r'$E_{bg}$ = '+str(np.around(vars()[files1A[i]+'Intercept'][0],2))+r'eV', linestyle = '--', color = 'darkred')
    ...
    ax2.tick_params(axis='both', which='major', pad=axesFontPadding-4)
