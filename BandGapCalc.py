...
for i in range(len(files)):
    ...
    vars()[files[i]+'hv'] = np.array(ListExtract(vars()[files[i]+'_Data'],2))
    vars()[files[i]+'Alpha'] = np.array(ListExtract(vars()[files[i]+'_Data'],3))
...
for i in range(len(files)):
    ...
    vars()[files[i]+'DirectAllowed'] = (vars()[files[i]+'Alpha']*vars()[files[i]+'hv'])**2
    vars()[files[i]+'IndirectAllowed'] = (vars()[files[i]+'Alpha']*vars()[files[i]+'hv'])**0.5
    vars()[files[i]+'DirectDisallowed'] = (vars()[files[i]+'Alpha']*vars()[files[i]+'hv'])**(2/3)
    vars()[files[i]+'IndirectDisallowed'] = (vars()[files[i]+'Alpha']*vars()[files[i]+'hv'])**(1/3)
    
    
    vars()[files[i]+'ticks'] = np.arange(min(vars()[files[i]+'hv']),max(vars()[files[i]+'hv'])+0.05,0.05)
...
for i in range(len(files)):
    ...
    # plt.plot(vars()[files[i]+'hv'],vars()[files[i]+'Alpha'])
    # plt.ylabel('α (cm^-1)')
    # plt.xlabel('hv (eV)')
    # # plt.xticks(vars()[files[i]+'ticks'],rotation = 90,fontsize=5)
    # plt.plot()
    
    # plt.figure(i+len(files)+1*(len(files))+1)
    # plt.minorticks_on()
    # plt.grid(b=True, which='major', color='k', linestyle='-')
    # plt.grid(b=True, which='minor', color='darkgray', linestyle='--')
    # plt.title(files[i]+'_Indirect_Disallowed')
    # plt.plot(vars()[files[i]+'hv'],vars()[files[i]+'IndirectDisallowed'])
    # plt.ylabel('α (cm^-1)')
    # plt.xlabel('hv (eV)')
    # plt.xticks(vars()[files[i]+'ticks'],rotation = 90,fontsize=5)
    # plt.plot()
