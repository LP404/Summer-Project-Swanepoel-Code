...
for i in range(len(files)):
    ...
    # vars()['xNewMax'+str(i)], vars()['yNewMaxUnCorr'+str(i)],vars()['xNewMin'+str(i)], vars()['yNewMinUnCorr'+str(i)] = F.FindAntiNode(vars()[files[i]+'T'][0],vars()['yFiltered'+str(i)])
...
for i in range(len(files)):
    ...
    vars()['nForMin'+str(i)] =  np.nan_to_num(vars()['nForMin'+str(i)], copy=True, nan=0.0, posinf=None, neginf=None)
    vars()['nForMax'+str(i)] = np.nan_to_num(vars()['nForMax'+str(i)], copy=True, nan=0.0, posinf=None, neginf=None)
    vars()['n1UncertMax'+str(i)] =  np.nan_to_num(vars()['n1UncertMax'+str(i)], copy=True, nan=0.0, posinf=None, neginf=None)
    vars()['n1UncertMin'+str(i)] =  np.nan_to_num(vars()['n1UncertMin'+str(i)], copy=True, nan=0.0, posinf=None, neginf=None)
    ...
    d1Combi = np.nan_to_num(d1Combi, copy=True, nan=0.0, posinf=None, neginf=None)
    ...
    d1ErrCombi = np.nan_to_num(d1ErrCombi, copy=True, nan=0.0, posinf=None, neginf=None)
    ...
    d2Combi = np.nan_to_num(d2Combi, copy=True, nan=0.0, posinf=None, neginf=None)
    d2ErrCombi = np.nan_to_num(d2ErrCombi, copy=True, nan=0.0, posinf=None, neginf=None)
    ...
    for k in range(len(LambdaCombi)):
        IndexVal = np.where(F.FindNearestVal(np.around(xP,1),LambdaCombi[k]) == np.around(xP,1))[0][0]
