...
for i in range(len(files)):
    # plt.figure(i,figsize=(29.7/2.54,21.0/2.54), dpi=600)
    plt.figure(0,dpi=600,figsize = ((FigHeight + int(FigHeight/2)),FigHeight))
    gc.collect()
    plt.minorticks_on()
    plt.grid(which='major', color='k', linestyle='-')
    plt.grid(which='minor', color='darkgray', linestyle='--')
    ...
    plt.semilogy(xVals,yVals, label = files[i], color = (Red[i]/255,0,Blue[i]/255), lw = LineW)
...
plt.legend(fontsize = LegSize, loc='center left', bbox_to_anchor=(1.0, 0.5), frameon=False,handlelength=1, ncols = 1)
